...
export const ScoresItems: FunctionComponent<{ playlistIds: string[]; scoresDB: ScoresDBPropsGrouped[] }> = ({ playlistIds, scoresDB }): ReactElement => {
  ...
  return (
    <div className='flex w-full flex-wrap'>
      {!scoresDB.length && <h3 className='font-alba text-orange-500 text-3xl mt-10'>You don&apos;t have any games records yet</h3>}
      {scoresDB?.map((info: ScoresDBPropsGrouped) => {
        ...
        return (
          <div className='w-full md:w-1/3 min-w-64 flex' key={id}>
            <div className='w-1/3 max-w-24'>{playlist && <PlaylistItem playlist={playlist} />}</div>
            <div className='w-2/3 pl-4 pt-6'>
              <h3 className='font-alba text-orange-500 text-xl -mb-2'>{name?.length > 12 ? `${name?.slice(0, 12)}...` : name}</h3>
